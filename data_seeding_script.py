...
def fetch_insert_data_set(conf, result_set):
        ...
        for doc in documents:                             # looping through all the documents of all the doctypes
            ...
            doctype = conn2.get_doc(conf["doctype"], doc["name"]).get("doctype")                        # getting the doctype of the document
            doc_already_exists_name = conn2.get_value(conf["doctype"], {"name": doc["name"]})         # getting the name of the document that already exists
            match not doc_already_exists:                                # handling the duplicate documents with Match Case this feature is new in python 3.10 
                  case False:                   # False here means if the document already exists
